...
class GetRoom(APIView):
    ...
    def get(self, request, format=None):
        ...
        if code != None:
            room = Room.objects.filter(code__iexact=code)
            if (len(room)) > 0:
        ...
        if serializer.is_valid():
            ...
            if not rooms.exists():
                ...
                if room.host != user_id:
                    return Response("You are not the host of this room.", status=status.HTTP_401_UNAUTHORIZED)
                else:
                    room.guest_can_pause = guest_can_pause
                    room.votes_to_skip = votes_to_skip
                    room.save(
                    ...
                    return Response(UpdateRoomSerializer(room).data, status=status.HTTP_201_CREATED)
        else:
